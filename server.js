...
var _methods = {
  ...
  _nodeInit: function (data) { /*初始化*/
    this._nodeFor(data)
  },
  _nodeFor: function (data) {
    var self = this
    for (var i = 0; i < data.length; i++) {
      (function (data) {
        ...
        fs.readFile('conf/vhost/' + data.name + '.conf', 'utf8', function (err) {
          if (!err) { // 如果有该文件则先删除
            fs.unlinkSync('conf/vhost/' + data.name + '.conf', mkDir.val, 'utf8', function (err) {
              if (err) {
                return console.error(err)
              }
            })
            fs.appendFile('conf/vhost/' + data.name + '.conf', mkDir.val, 'utf8', function (err) {
              if (err) {
                return console.error(err)
              }
            })
